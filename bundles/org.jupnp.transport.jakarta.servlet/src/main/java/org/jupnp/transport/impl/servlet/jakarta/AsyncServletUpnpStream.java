 ...
 */
package org.jupnp.transport.impl.servlet.jakarta;
...
/**
 * Implementation based on the jakarta.servlet API, which -- unlike javax.servlet -- has required async
 * support unconditionally since Servlet 5.0, so there's no non-async fallback to consider here.
 * <p>
 * Concrete implementations must provide a connection wrapper, as this wrapper most likely has to access
...
public abstract class AsyncServletUpnpStream extends ServletUpnpStream implements AsyncListener {
    ...
    protected void complete() {
        try {
            ...
            // If the underlying connection, for whatever reason, is in an illegal state, this will be thrown
