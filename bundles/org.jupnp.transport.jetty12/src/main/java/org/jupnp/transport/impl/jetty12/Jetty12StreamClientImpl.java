...
public class Jetty12StreamClientImpl extends AbstractStreamClient<StreamClientConfigurationImpl, Request> {
    ...
    public Jetty12StreamClientImpl(StreamClientConfigurationImpl configuration) throws InitializationException {
    ...
    protected Request createRequest(StreamRequestMessage requestMessage) {
        ...
        switch (upnpRequest.getMethod()) {
            case POST:
            case NOTIFY:
                request.body(createRequestContent(requestMessage));
                break;
            default:
        }
        ...
        if (requestMessage.getOperation().getHttpMinorVersion() == 0) {
            ...
            request.headers(fields -> fields.add(HttpHeader.CONNECTION, "close"));
        }
    ...
    protected <O extends UpnpOperation> Request.Content createRequestContent(final UpnpMessage<O> upnpMessage) {
