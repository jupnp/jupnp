...
import org.jupnp.transport.javax.servlet.ServletContainerAdapter;
 ...
 */
public class Jetty9ServletContainer implements ServletContainerAdapter {

    private final Logger logger = LoggerFactory.getLogger(Jetty9ServletContainer.class.getName());

    // Singleton
    public static final Jetty9ServletContainer INSTANCE = new Jetty9ServletContainer();

    protected Server server;

    private Jetty9ServletContainer() {
        resetServer();
    }
    ...
    public synchronized int addConnector(String host, int port) throws IOException {
        ...
        try {
            ...
            // connector.open() above already bound the server socket; close it explicitly so the port
            // isn't leaked if anything after that point fails.
            connector.close();
